---
title: "Choose Your Adventure - How to choose the right visualization for your data"
author: "Gee Hyun Kwon (gk2575), Simei Li (sl4525)"
output:
  html_document:
    df_print: paged
---
...
covid_data <- as.data.frame(read_excel("covid.xlsx"))
# date conversion for aggregation later
covid_data$dateRep <- as.Date(covid_data$dateRep, format = "%y-%m-%d")
df = covid_data
```
...
names(df)[names(df) == 'dateRep'] <- 'date'
countries <- c("France", "Germany", "Italy", "Sweden", "Romania")
df = df[df$country %in% countries, ]
df = df[(df$date>= "2020-04-06" & df$date<= "2020-04-12"),]
df %>% select(date, cases, country)
```
...
g <- ggplot(df, aes(date, cases, color = country)) + geom_line() +
...
ggplot(df, aes(fill=country, y=cases, x=date)) + 
